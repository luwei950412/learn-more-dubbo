...
(function ($) {
    ...
    var DanmuPlayer = function (element, options) {
        ...
        this.$ctrlMain.append('<div class="opacity ctrl-btn-right"><i class="icon-volume" ></i><input class="ctrl-btn-right danmu-op" value="100" type="range" /></div>');
        ...
        //调整透明度事件
        $(this.id + " .danmu-op").on('mouseup touchend', {video: this.video, that: that}, function (e) {
            // $(e.data.that.id + " .danmu-div").data("opacity", (e.target.value / 100));
            // $(e.data.that.id + " .danmaku").css("opacity", (e.target.value / 100));

            e.data.video.volume=e.target.value / 100;
